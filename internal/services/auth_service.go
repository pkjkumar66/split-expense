...
func (s *AuthService) SignUp(req models.SignUpRequest) (*models.AuthResponse, error) {
	...
	user := models.User{}
	query := `
		INSERT INTO users (id, email, password_hash, name, default_currency)
		VALUES ($1, $2, $3, $4, $5)
		RETURNING id, email, name, default_currency, created_at, updated_at
	`
	err = s.db.QueryRow(query, userID, req.Email, string(hashedPassword), req.Name, req.DefaultCurrency).Scan(
